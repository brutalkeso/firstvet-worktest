...
export function sendSuccess<T>(response: T, res: ServerResponse): Promise<PathEnd> {
    res.status(200).send(response)
    return Promise.resolve(PathEndConst)
}

export function sendEmptySuccess(res: ServerResponse): Promise<PathEnd> {
    res.status(200).send()
    return Promise.resolve(PathEndConst)
}

export function sendFailure(res: ServerResponse, code: number, value?: any): Promise<PathEnd> {
    res.status(code).send(value)
    return Promise.resolve(PathEndConst)
}